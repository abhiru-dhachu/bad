...
async ParseButtonImage(url) {
    let img = await prepareWAMessageMedia({ image: { url: url } }, { upload: this.client.waUploadToServer });
    return img.imageMessage || null;
}
async ParseButtonVideo(url) {
    let img = await prepareWAMessageMedia({ video: { url: url } }, { upload: this.client.waUploadToServer });
    return img.videoMessage || null;
}
